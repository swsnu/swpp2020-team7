import { Avatar, Button, createStyles, Grid, makeStyles, Theme, Input } from '@material-ui/core';
					...
					<Grid style={{ display: 'flex', flexDirection:'column'}}>
						<Grid id="comment-header">
							<Grid id="comment-author">
								<div style={{ marginRight: 10, textAlign: 'left' }}>{comment?.author}</div>
								<div style={{ textAlign: 'left', color: 'gray' }}>{comment?.createdAt}</div>
							</Grid>
							<Grid id="comment-info-box">
								...
								{user!.username === comment?.author && (
									<Grid >
										...
										<Collapse in={alert}>
											<Alert id="comment-setting-alert" 
												onMouseOver={() => setAlert(true)}
												onMouseLeave={() => setAlert(false)}
												icon={false}>
												<Button id="comment-edit" onClick={() => onClickEditComment()}>
													수정
												</Button>
												<Button
													id="comment-delete"
